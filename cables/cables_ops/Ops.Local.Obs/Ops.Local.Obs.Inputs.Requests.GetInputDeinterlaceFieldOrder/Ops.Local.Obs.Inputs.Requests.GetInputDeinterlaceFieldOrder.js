const
    ...
    outInputDeinterlaceFieldOrder = op.outString("Deinterlace Field Order");
...
inCall.onTriggered = () => {
    ...
    if (obs && typeof obs.call === 'function') {
        ...
        obs.call('GetInputDeinterlaceFieldOrder', params)
            .then((data) => {
                if (data) {
                    outInputDeinterlaceFieldOrder.set(data.inputDeinterlaceFieldOrder || "");
                    outTrigger.trigger();
                }
            })
            .catch((err) => {
                op.logError("OBS Request Error (GetInputDeinterlaceFieldOrder):", err);
            });
    } else {
