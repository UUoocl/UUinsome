const
    ...
    outInputAudioBalance = op.outNumber("Audio Balance");
...
inCall.onTriggered = () => {
    ...
    if (obs && typeof obs.call === 'function') {
        ...
        obs.call('GetInputAudioBalance', params)
            .then((data) => {
                if (data) {
                    outInputAudioBalance.set(data.inputAudioBalance || 0);
                    outTrigger.trigger();
                }
            })
            .catch((err) => {
                op.logError("OBS Request Error (GetInputAudioBalance):", err);
            });
    } else {
