const
    ...
    outInputDeinterlaceMode = op.outString("Deinterlace Mode");
...
inCall.onTriggered = () => {
    ...
    if (obs && typeof obs.call === 'function') {
        ...
        obs.call('GetInputDeinterlaceMode', params)
            .then((data) => {
                if (data) {
                    outInputDeinterlaceMode.set(data.inputDeinterlaceMode || "");
                    outTrigger.trigger();
                }
            })
            .catch((err) => {
                op.logError("OBS Request Error (GetInputDeinterlaceMode):", err);
            });
    } else {
