const
    ...
    inInputDeinterlaceMode = op.inString("Deinterlace Mode", "OBS_DEINTERLACE_MODE_DISABLE"),
...
inCall.onTriggered = () => {
    ...
    if (obs && typeof obs.call === 'function') {
        const params = {
            inputDeinterlaceMode: inInputDeinterlaceMode.get()
        };
        ...
        obs.call('SetInputDeinterlaceMode', params)
            .then(() => {
                outTrigger.trigger();
            })
            .catch((err) => {
                op.logError("OBS Request Error (SetInputDeinterlaceMode):", err);
            });
    } else {
