const
    ...
    inFilterEnabled = op.inBool("Filter Enabled", true),
...
inCall.onTriggered = () => {
    ...
    if (obs && typeof obs.call === 'function') {
        ...
        const params = {
            ...
            filterEnabled: inFilterEnabled.get()
        };
        ...
        obs.call('SetSourceFilterEnabled', params)
            .then(() => {
                outTrigger.trigger();
            })
            .catch((err) => {
                op.logError("OBS Request Error (SetSourceFilterEnabled):", err);
            });
    } else {
