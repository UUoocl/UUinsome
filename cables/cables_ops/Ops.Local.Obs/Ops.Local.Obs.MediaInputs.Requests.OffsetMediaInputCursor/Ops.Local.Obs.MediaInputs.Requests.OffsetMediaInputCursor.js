const
    ...
    inMediaCursorOffset = op.inNumber("Media Cursor Offset (ms)", 0),
    outTrigger = op.outTrigger("Finished");

inCall.onTriggered = () => {
    const obs = inConnection.get() || globalThis.obs;
    if (obs && typeof obs.call === 'function') {
        const offset = inMediaCursorOffset.get();
        ...
        obs.call('OffsetMediaInputCursor', params)
            .then(() => {
                outTrigger.trigger();
            })
            .catch((err) => {
                op.logError("OBS Request Error (OffsetMediaInputCursor):", err);
            });
    } else {
