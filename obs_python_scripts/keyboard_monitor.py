...
def update_browser(text):
    """Updates the browser source with the current hotkey text."""
    source = obs.obs_get_source_by_name(g_source_name)
    if source is not None:
        payload = json.dumps({"key": text})
        
        cd = obs.calldata_create()
        obs.calldata_set_string(cd, "eventName", g_event_name)
        obs.calldata_set_string(cd, "jsonString", payload)
        
        handler = obs.obs_source_get_proc_handler(source)
        obs.proc_handler_call(handler, "javascript_event", cd)
...
def script_description():
    ...
    return "Show pressed key \nSelect a Browser Source to receive the key event."

def script_properties():
    """Defines the properties the user can change."""
    props = obs.obs_properties_create()

    # Add list of available browser sources
...
def script_update(settings):
    ...
    global g_source_name, g_event_name
    g_source_name = obs.obs_data_get_string(settings, "source")
    g_event_name = obs.obs_data_get_string(settings, "event_name")

def script_defaults(settings):
    """Called to set the default settings."""
    obs.obs_data_set_default_string(settings, "event_name", "keyboard_event")
